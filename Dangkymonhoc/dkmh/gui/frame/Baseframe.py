...
class Baseframe(Frame):
    ...
    def __init__(self, gui, window, wid, hei, type):
        super().__init__()
#         self.pack()
        self.gui = gui
        self["height"] = hei
        ...
        self["bg"] = "#f7f5f0"
        if type==1:
            self.gui.resize(1000, 600)
            self.userfr = Userframe(self.gui, self.gui.getWid(), self.gui.getHei())
            self.userfr.grid(row=0, column=0)
        else:
            self.gui.resize(1200, 700)
